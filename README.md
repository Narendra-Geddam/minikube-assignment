...
- Python 3, `boto3`, and Ansible collection `amazon.aws` installed on the Jenkins agent (for dynamic inventory)
...
Ansible uses the `amazon.aws.aws_ec2` inventory plugin via `ansible/aws_ec2.yaml`.
Defaults:
